...
from db.mediapulse import fetch_ticker_profile
...
class TickerProfileProvider(ContextProvider):
    ...
    async def before_run(
        ...
        ticker = (context.input_messages[-1].text if context.input_messages else "").strip()

        if not ticker:
            return

        try:
            profile = await asyncio.to_thread(fetch_ticker_profile, ticker)
        except Exception:
            profile = None
        ...
        context.extend_instructions(
            self.source_id,
            f"The subject '{ticker}' is a listed stock ticker. Verified company details from the exchange "
