...
_SUBSCRIPTIONS_SQL = """
    select u.email, u.name, t.symbol, t.name as ticker_name
...
def fetch_subscriptions(symbol: str | None = None, email: str | None = None) -> list[dict]:
    """Return active subscriptions as dicts: email, name, symbol, ticker_name. Read-only.

    Pass `symbol` to limit to one ticker's subscribers, or `email` to limit to one user.
    """
...
def fetch_ticker_profile(symbol: str) -> dict[str, str] | None:
    """Return a curated company profile for a ticker symbol, or None if unknown. Read-only.

    Reads the IDX listing metadata (sector taxonomy, main business, headquarters, website,
    listing board and date) so the analyst has real context beyond the bare symbol. Returns
    None when the symbol is blank, the database is not configured, or the symbol is not listed.
    """
    if not (symbol and symbol.strip() and os.getenv("MEDIAPULSE_DATABASE_URL")):
        return None

    with psycopg.connect(_conninfo(), connect_timeout=15) as conn:
        conn.read_only = True
        with conn.cursor() as cur:
            cur.execute(_TICKER_SQL, [symbol.strip()])
            row = cur.fetchone()
    if not row:
        return None
